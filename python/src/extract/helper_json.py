...
def to_JSON(
    ...
    {"table_name":[{"col0":value,"col1"...:value,"coln-1":value},{"col0":value,...,"coln-1":value},...,{"col0":value,...,"coln-1":value}]}
    ...
    Args:
        ...
        from_time (str | None, optional): early limit of the extraction. Defaults to None.
        to_time (str | None, optional): last extraction time. Defaults to None.
...
def name_rows(rows: list, column_names: list) -> list:
    ...
    for row in rows:
        if len(row) != len(column_names):
            raise LengthMissMatchException(
                f"length of row does not match number of columns:{row},{column_names}"
            )
...
def extract_names_from_columns_data(columns_data: list) -> list:
    ...
    ]  # Are there any circumstances where name is not in the dictionary? PG8000's doc are terrible
