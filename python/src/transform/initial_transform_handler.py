...
def lambda_handler(event, context):
    ...
    try:
        ...
        timestamp = event.get("timestamp",datetime.now(tz=pytz.UTC).strftime("%Y-%m-%d_%H:%M:%S.%f")[:-3])
...
def transform_and_combine(table_name, s3, raw_bucket, key,secondary_key, seconday_table, transform_func) -> pd.DataFrame:
...
def transform_tables(tables:list, s3,timestamp:str) -> dict:
    ...
    if "address" in tables:
        ...
        transformed_tables["dim_location"] = transform_table("address", s3, RAW_DATA_BUCKET, key,transform_location)
    if "design" in tables:
        key = make_key("design",timestamp)
        transformed_tables["dim_design"] = transform_table("design", s3, RAW_DATA_BUCKET, key,transform_design)
    if "payment_types" in tables:
        key = make_key("payment_types",timestamp)
        transformed_tables["dim_payment_types"] = transform_table("payment_types", s3, RAW_DATA_BUCKET, key,transform_payment_type)
    if "transaction" in tables:
        key = make_key("transaction",timestamp)
        transformed_tables["dim_transaction"] = transform_table("transaction", s3, RAW_DATA_BUCKET, key,transform_transaction)
    if "counterparty" in tables:
        if "address" in tables:
            key_counterparty = make_key("counterparty",timestamp)
            key_address = make_key("address",timestamp)
            transformed_tables["dim_counterparty"] = transform_and_combine("counterparty",s3,RAW_DATA_BUCKET,key_counterparty,key_address,"address",transform_counterparty)
        else:
            raise ValueError("address table missing unable to transform counterparty table")
    if "staff" in tables:
        if "department" in tables:
            staff_key = make_key("staff",timestamp)
            department_key = make_key("department",timestamp)
            transformed_tables["dim_staff"] = transform_and_combine("staff",s3,RAW_DATA_BUCKET,staff_key,department_key,"department",transform_staff_with_department)
        else:
    ...
    if "sales" in tables:
        ...
        transformed_tables["fact_sales_order"] = transform_table("sales",s3,RAW_DATA_BUCKET,sales_key,"sales",sales_facts)
